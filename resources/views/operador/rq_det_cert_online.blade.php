    ...
    @foreach($rqAsig as $key => $value1) 
			...
			<input type="hidden" value="{{ $nombre_fase }}" name="nom_fase" id="nom_fase" >
				@foreach($arrayTiempoFin as $keyt => $valuet) 
					@if($valuet['id_rq'] == $value1->id_requerimiento)
						<!--{{ $aux=1 }}-->	
						<input type="button" name='tarea_{{ $value1->id_requerimiento }}' id='tarea_{{ $value1->id_requerimiento }}' value="DETENER TAREA CERTIFICACION ONLINE" onclick="iniciarFinalizar(this,{{ $value1->id_requerimiento }});"> 
						<input type="hidden" value="{{$valuet['id_tp']}}" name="tiempo_i_{{ $value1->id_requerimiento }}" id="tiempo_i_{{ $value1->id_requerimiento }}" >
				    @endif
				@endforeach
				@if($aux==0)
					<input type="button" name='tarea_{{ $value1->id_requerimiento }}' id='tarea_{{ $value1->id_requerimiento }}' value="INICIAR TAREA CERTIFICACION ONLINE" onclick="iniciarFinalizar(this,{{ $value1->id_requerimiento }});">
					<input type="hidden" value="0" name="tiempo_i_{{ $value1->id_requerimiento }}" id="tiempo_i_{{ $value1->id_requerimiento }}" >
				@endif
			@endif
			</div>
		...
		<div class="card-body">
          ...
          <div class="row">
            ...
            <hr>                  
            @include('operador.listado_adjunto_inst')
            <br>
            <br>
            <br>
            @include('operador.subir_adjunto_etapas')
            <div class="row">
            @if($adj_vacio == 1)
            <div class="col-lg-6 text-right">  
                ...
                <a href="#" class="btn btn-success btn-icon-split" data-toggle="modal" data-target="#textoModal_{{ $value1->id_requerimiento }}">
                  ...
                  <span class="text">Certificar Online</span>
                </a>
              </div>
             @else
             <div class="col-lg-6 text-right">  
             ...
             @endif
              ...
              <div class="col-lg-6 text-left">  
                <div class="my-2"></div>
                  <a class="btn btn-primary" href="{{ route('revCertificacionOnLine') }}">
                  Cerrar ventana
                  </a>
              </div>
            </div>
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
              <form id="logout-form1_{{ $value1->id_requerimiento }}" action="{{ route('guadarCertOnline', $value1->id_requerimiento ) }}" method="POST" >
                  {{ csrf_field() }}
	              ...
	              <div class="col-lg-12 text-left">
	                  <textarea class="form-control" name="textDesc" rows="4" id="textDesc_{{ $value1->id_requerimiento }}" required pattern="[A-Za-z0-9]" placeholder="Escriba conformidad de la certifiación Online."></textarea>
	              </div>
              </form>
            </div>
